...
export async function POST(
  request: Request,
  context: { params: { pipelineId: string; batchSessionId: string } }
) {
  try {
    const payload = await request.text();
    const response = await fetch(
      `${API_BASE_URL}/noodle/pipelines/${context.params.pipelineId}/batch-sessions/${context.params.batchSessionId}/resume`,
      {
        method: "POST",
        headers: {
