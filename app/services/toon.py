...
def from_toon(text: str) -> Any:
    ...
    for line_number, row in enumerate(rows[1:], start=2):
        ...
        parsed_value = _parse_payload(kind, payload, line_number)
        nodes[path] = _Node(kind=kind, value=parsed_value, line_number=line_number)
...
def toon_to_json(toon_text: str, indent: int | None = None) -> str:
    value = from_toon(toon_text)
    return json.dumps(value, separators=(",", ":"), indent=indent, ensure_ascii=False)
...
def _build_tree(
    ...
    node = nodes[path]
    child_rows = children.get(path, [])
    child_rows.sort(key=lambda row: row[2])
...
def _parse_path(path_text: str, line_number: int) -> tuple[str, ...]:
    ...
    parts = path_text[1:].split("/")
    decoded: list[str] = []
    for part in parts:
        decoded.append(_decode_path_segment(part, line_number))
