...
def create_pipeline(payload: PipelineCreate, service: PipelineService = Depends(get_pipeline_service)) -> PipelineRead:
    return service.create_pipeline(payload)


@router.get("/{pipeline_id}", response_model=PipelineRead)
def get_pipeline(pipeline_id: str, service: PipelineService = Depends(get_pipeline_service)) -> PipelineRead:
    try:
...
def update_pipeline(pipeline_id: str, payload: PipelineUpdate, service: PipelineService = Depends(get_pipeline_service)) -> PipelineRead:
    try:
...
def validate_pipeline(pipeline_id: str, service: PipelineService = Depends(get_pipeline_service)) -> list[ValidationIssue]:
    try:
...
def publish_pipeline(pipeline_id: str, service: PipelineService = Depends(get_pipeline_service)) -> PipelineRead:
    try:
...
def run_pipeline(pipeline_id: str, service: RunService = Depends(get_run_service)) -> PipelineRunRead:
    try:
...
def list_pipeline_runs(pipeline_id: str, service: RunService = Depends(get_run_service)) -> list[PipelineRunRead]:
