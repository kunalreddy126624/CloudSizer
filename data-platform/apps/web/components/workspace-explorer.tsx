  ...
  return (
    <aside className="flex h-full min-h-0 flex-col rounded-[28px] border border-slate-200 bg-white/90 p-4 shadow-platform">
      <div className="mb-4">
        ...
        <h2 className="mt-1 text-lg font-semibold text-slate-900">{reposQuery.data?.[0]?.name ?? "Repository"}</h2>
        <p className="text-sm text-slate-500">{reposQuery.data?.[0]?.description ?? "Repository explorer"}</p>
      </div>
